...
void* horizontalCheck(void *param){
  ...
  for(a=0; a<9; a++){
	...
	printf("A linha %d contem mais de uma ocorrencia do numero %c.\n", p.value+1, p.matrix[p.value][a]);
	totalErrors++;
	break;
      }
    }
...
void* verticalCheck(void *param){
  ...
  for(a=0; a<9; a++){
	...
	printf("A coluna %d contem mais de uma ocorrencia do numero %c.\n", p.value+1, p.matrix[a][p.value]);
	totalErrors++;
	break;
      }
    }
...
void* submatrixCheck(void *param){
  ...
  for(a=0; a<9; a++){
	...
	printf("A submatriz %d contem mais de uma ocorrencia do numero %c.\n", p.value+1, buffer[a]);
	totalErrors++;
	break;
      }
    }
